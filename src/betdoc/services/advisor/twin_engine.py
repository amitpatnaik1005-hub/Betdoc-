...
class TwinSessionResult(BaseModel):
    ...
    def actionable(self) -> tuple[TwinRecommendation, ...]:
        ...
        return tuple(item for item in self.recommendations if item.action_type.requires_stake)

    @property
    def refusals(self) -> tuple[TwinRecommendation, ...]:

        return tuple(item for item in self.recommendations if item.action_type is ActionType.PASS)
...
class TwinAdvisorService:
    ...
    def evaluate_session(
        ...
        *,
        ...
        committed_by_sport: dict[SportType, int] = dict(account_state.exposure_by_sport_paise)
        ...
        forecast = self._build_forecast(
            ...
            active_news_alerts=len([item for item in modifiers if item.is_active(now=moment)]),
            trace_id=resolved_trace,
            now=moment,
        )
    ...
    def remaining_loss_budget_paise(
        *,
        ...
        budget = (
            profile.daily_loss_limit_paise - realized_loss - account_state.unsettled_exposure_paise
        )
    ...
    def _evaluate_opportunity(
        ...
        *,
        ...
        if opportunity.is_parlay and opportunity.leg_count > profile.max_parlay_legs:
            return refuse(
                RejectionReason.PARLAYS_DISABLED,
                f"{opportunity.leg_count} legs exceeds your maximum of {profile.max_parlay_legs}",
            ), None
        ...
        variance = (
            adjusted_probability * (1.0 - adjusted_probability) * (opportunity.offered_odds**2)
        )

        required_edge = self._config.min_stake_fraction * uncertainty_penalty * variance
        ...
        sport_headroom = max(sport_cap - committed_by_sport.get(opportunity.sport_type, 0), 0)
        ...
        entry.info(
            ...
            binding_constraint=binding,
            uncertainty_penalty=round(uncertainty_penalty, 4),
            stake_reduction_from_news=round(recommendation.stake_reduction_from_news, 4),
    ...
    def _build_forecast(
        ...
        *,
        ...
        counts = Counter(
            item.rejection_reason for item in recommendations if item.rejection_reason is not None
        )
        ...
        if any(reason.is_hard_capital_constraint for reason in rejections):
            warnings.append(
    ...
    def _halted_session(
        ...
        *,
        ...
        forecast = SessionForecast(
            ...
            active_news_alerts=len([item for item in modifiers if item.is_active(now=now)]),
        ...
        return TwinSessionResult(forecast=forecast, recommendations=refusals, sizing_audit=())
